...
auto CFont::InitFont( IFW1Factory* pFW1Factory , std::wstring Name , float FontSize ) -> void
{
	...
	if ( FAILED( pFW1Factory->CreateFontWrapper( GetExiliumGUI()->GetDevice() , NULL , &createParams , &m_pFontWrapper ) ) )
	{
...
auto CFont::DrawString( int x , int y , ImColor Color , int Flags , const char* fmt , ... ) -> void
{
	...
	if ( m_pFontWrapper )
	{
		...
		{
			m_pFontWrapper->DrawString( GetExiliumGUI()->GetDeviceContext() ,
