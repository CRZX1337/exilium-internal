...
auto CExiliumGUI::OnInit( IDXGISwapChain* pSwapChain ) -> void
{
	DXGI_SWAP_CHAIN_DESC SwapChainDesc;

	if ( FAILED( pSwapChain->GetDevice( IID_PPV_ARGS( &m_pDevice ) ) ) )
	{
		DEV_LOG( "[error] CExiliumGUI::OnInit: #1\n" );
		return;
	}
	...
	{
		DEV_LOG( "[error] CExiliumGUI::OnInit: #2\n" );
		return;
	}
...
auto CExiliumGUI::OnDestroy() -> void
{
	SetWindowLongPtrA( m_hCS2Window , GWLP_WNDPROC , (LONG_PTR)GetExiliumGUI()->m_WndProc_o );
...
auto CExiliumGUI::InitFont() -> void
{
...
void CExiliumGUI::OnResizeBuffers( IDXGISwapChain* pSwapChain )
{
	OnDestroy();
}
 
void CExiliumGUI::OnRender( IDXGISwapChain* pSwapChain )
{
...
auto CExiliumGUI::OnReopenGUI() -> void
{
...
LRESULT WINAPI CExiliumGUI::GUI_WndProc( HWND hwnd , UINT uMsg , WPARAM wParam , LPARAM lParam )
{
	...
	if ( GetExiliumGUI()->m_bInit )
	{
		if ( uMsg == WM_KEYUP && wParam == VK_INSERT )
			GetExiliumGUI()->OnReopenGUI();

		if ( GetExiliumGUI()->IsVisible() && ImGui_ImplWin32_WndProcHandler( hwnd , uMsg , wParam , lParam ) == 0 )
			return true;
	}

	return CallWindowProcA( GetExiliumGUI()->m_WndProc_o , hwnd , uMsg , wParam , lParam );
}

auto CExiliumGUI::SetIndigoStyle() -> void
{
...
auto CExiliumGUI::SetVermillionStyle() -> void
{
	auto& style = ImGui::GetStyle();
	auto& colors = style.Colors;
...
auto CExiliumGUI::SetClassicSteamStyle() -> void
{
...
auto CExiliumGUI::UpdateStyle() -> void
{
	ImGui::SetCurrentContext( m_pImGuiContext );

	if ( Settings::Menu::MenuStyle == EExiliumGuiStyle_t::EXILIUM_GUI_STYLE_INDIGO )
		SetIndigoStyle();
	else if ( Settings::Menu::MenuStyle == EExiliumGuiStyle_t::EXILIUM_GUI_STYLE_VERMILLION )
		SetVermillionStyle();
	else if ( Settings::Menu::MenuStyle == EExiliumGuiStyle_t::EXILIUM_GUI_STYLE_CLASSIC_STEAM )
		SetClassicSteamStyle();
}

bool CExiliumGUI::FreeTypeBuild::PreNewFrame()
{
	if ( !WantRebuild )
		return false;
...
auto CExiliumGUI::ClearRenderTargetView() -> void
{
	if ( m_pRenderTargetView )
	{
...
auto GetExiliumGUI() -> CExiliumGUI*
{
	return &g_ExiliumGUI;
}
